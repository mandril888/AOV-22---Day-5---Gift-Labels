<template>
  <div class="w-full mt-10 mb-5 flex justify-center items-center">
    <div>
      <div class="w-full max-w-2xl p-4 rounded bg-red-400 text-white">
        <div class="border-2 rounded flex flex-col md:flex-row md:items-center md:gap-8 p-4">
          <img src="/assets/santa.svg" alt="" class="max-w-[12em]" />
          <div>
            <h1 class="font-serif text-4xl">Happy Holidays!</h1>
            <div class="mt-4 bg-white text-black p-2 rounded">TO: {{ to }}</div>
            <div class="mt-2 bg-white text-black p-2 rounded">FROM: {{ from }}</div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="w-full flex justify-center items-center">
    <div>
      <div class="w-full max-w-2xl p-4 bg-gray-300 rounded">
        <div>
          <label
            >To:
            <input type="text" v-model="to" class="w-full my-1 bg-white text-black p-2 rounded" />
          </label>
        </div>
        <div>
          <label
            >From:
            <input type="text" v-model="from" class="w-full my-1 bg-white text-black p-2 rounded" />
          </label>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref } from 'vue'

const to = ref('John')
const from = ref('Joana')
</script>
